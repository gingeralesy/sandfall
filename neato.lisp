...
(defmethod paint ((neato neato) target)
  (let* ((world (world neato))
         ...
         (height (min (second (array-dimensions world))
                      (q+:height rect))))
    (with-finalizing ((image (q+:make-qimage width height (q+:qimage.format_argb32))))
      (q+:fill image #x00000000)
      (dotimes (x width)
        (dotimes (y height)
          (let ((element (aref world x y)))
            (when (and element (< 0 element))
              (let ((color (case element
                             ...
                             (2 #xFFFFFF00)
                             (T #xFFFFFFFF))))
                (setf (q+:pixel image x y) color))))))
      (q+:draw-image target 0 0 image))))
